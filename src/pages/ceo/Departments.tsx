...
import { DailyUpdate, WeeklyReport, Issue, TeamMember } from '@/types'
...
export function DepartmentDetail() {
  ...
  useEffect(() => {
    ...
    getTeamMembers(summary.department.id).then((r) => setTeam(r.data))
  }, [summary])
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="card p-5">
        ...
        {team.length === 0 ? (
          <p className="text-sm text-muted">No team members added by this department yet.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {team.map((m) => (
              <div key={m.id} className="flex items-center gap-2.5 bg-surface border border-border rounded-lg px-3 py-2">
