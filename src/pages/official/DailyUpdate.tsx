    ...
    <div className="flex flex-col gap-6 max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="card p-5 flex flex-col gap-5">
            ...
            {(['green', 'yellow', 'red'] as Status[]).map((s) => (
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-xs font-medium border capitalize ${
                  status === s ? 'bg-brand text-white border-brand' : 'border-border text-muted'
                }`}
              >
        ...
        <button
          type="submit"
          disabled={submitting}
          className="bg-brand text-white rounded-lg py-2.5 text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-60"
        >
