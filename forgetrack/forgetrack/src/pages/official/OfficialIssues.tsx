  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowForm((v) => !v)} className="flex items-center gap-1.5 bg-ink text-white text-sm font-medium rounded-lg px-3.5 py-2">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card p-5 flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="flex gap-2 mt-1.5">
              {(['Low', 'Medium', 'High', 'Critical'] as IssuePriority[]).map((p) => (
                <button type="button" key={p} onClick={() => setPriority(p)} className={`flex-1 py-1.5 rounded-lg text-xs font-medium border ${priority === p ? 'bg-ink text-white border-ink' : 'border-border text-muted'}`}>
                  {p}
                </button>
              ))}
            </div>
          ...
          <button type="submit" disabled={submitting} className="bg-ink text-white rounded-lg py-2.5 text-sm font-medium disabled:opacity-60">
